...
export default function AdminDashboard() {
  ...
  const fetchTransformers = useCallback(async () => {
    setLoading(true);
    try {
      // Use the API route (which uses direct postgres) instead of Supabase client
      // This avoids PostgREST PGRST002 schema cache errors
      const res = await fetch('/api/admin/transformers');
      const json = await res.json();
      if (!json.success) throw new Error(json.error || 'Failed to fetch transformers');
      setTransformers(json.data || []);
    } catch (err: any) {
      toast.error(err.message || 'Network error – could not fetch transformers');
    } finally {
      setLoading(false);
    }
  }, []);
